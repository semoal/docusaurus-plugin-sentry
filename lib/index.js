...
function pluginSentry(_, opts) {
    if (!opts) {
        throw new Error(`You need to specify an object with a key 'DSN' to use docusaurus-plugin-sentry.`);
    }
    const { DSN } = opts;
    if (!DSN) {
        throw new Error('You specified docusaurs-plugin-sentry options but the "DSN" field was missing. ' +
            'Please ensure this is not a mistake.');
    }
    if (DSN.includes("http")) {
        throw new Error('Your "DSN" property is not valid, should be without http|s URL' +
